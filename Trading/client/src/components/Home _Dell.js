...
onDelete=(id)=>{
  axios.delete(`/Dell/post/delete/${id}`).then((res)=>{
    ...
    return (
      <div className="container">
        <div className="row">
          ...
          <h4 class="text-primary bg-subtle">Dell</h4>
            </div>
          ...
          <tbody>
            {this.state.posts.map((posts,index)=>(
              <tr key={index}>
                ...
                <td>
                  <a href={`/Dell/post/${posts._id}`} style={{textDecoration:'none'}}>
                  {posts.orderID}
                  </a>
                </td>
                <td>{posts.symbol}</td>
                ...
                <td>{posts.status}</td>
                <td>
                <a className="btn btn-warning" href={`/Dell/edit/${posts._id}`}> 
