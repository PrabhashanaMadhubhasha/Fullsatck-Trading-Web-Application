...
onDelete=(id)=>{
  axios.delete(`/Asus/post/delete/${id}`).then((res)=>{
    ...
    return (
      <div className="container">
        <div className="row">
          ...
          <h4 class="text-primary bg-subtle">Asus</h4>
            </div>
          ...
          <tbody>
            {this.state.posts.map((posts,index)=>(
              <tr key={index}>
                ...
                <td>
                  <a href={`/Asus/post/${posts._id}`} style={{textDecoration:'none'}}>
                  {posts.orderID}
                  </a>
                </td>
                <td>{posts.symbol}</td>
                ...
                <td>{posts.status}</td>
                <td>
                <a className="btn btn-warning" href={`/Asus/edit/${posts._id}`}> 
